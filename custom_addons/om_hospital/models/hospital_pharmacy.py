...
class HospitalPharmacy(models.Model):
    """Class holdings hospital pharmacy details"""
    ...
    list_price = fields.Char(string="Prince List", help="Price list for the pharmacist")
    ...
    def create_sale_order(self, kwargs):
        ...
        if 'op' not in kwargs.keys():
            patient_id = self.env['hospital.patient'].sudo().search(
            ...
            pharmacy_sale_order.sudo().write({
                'order_line': [(0, 0, {
                    'product_id': self.env['product.product'].search(
                        [('product_tmpl_id', '=', int(rec['product']))]).id,
